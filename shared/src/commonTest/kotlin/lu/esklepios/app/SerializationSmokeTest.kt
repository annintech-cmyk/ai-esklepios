...
class SerializationSmokeTest {
    ...
    private val sampleSlotDto = AppointmentSlotDto(
        id = "slot_1", practitionerId = "p1",
        dateTime = "2026-06-01T09:00:00", isAvailable = true, durationMinutes = 30
    )
    private val sampleScheduleDto = ScheduleDayDto(
        dayOfWeek = "MONDAY", openTime = "08:00", closeTime = "17:00", isOpen = true
    )

    @Test fun `LoginRequest round-trips`() = assertRoundTrip(LoginRequest("a@b.lu", "secret"))

    @Test fun `LoginResponse round-trips`() = assertRoundTrip(
        LoginResponse(token = "jwt", refreshToken = "ref", user = sampleUserDto)
    )

    @Test fun `RegisterRequest round-trips`() = assertRoundTrip(
    ...
    @Test fun `RefreshTokenResponse round-trips`() = assertRoundTrip(
    ...
    @Test fun `PractitionerDto round-trips`() = assertRoundTrip(
        PractitionerDto(
    ...
    @Test fun `AppointmentDto round-trips`() = assertRoundTrip(
        AppointmentDto(
    ...
    @Test fun `CreateAppointmentRequest round-trips`() = assertRoundTrip(
        CreateAppointmentRequest(
            practitionerId = "p1", dateTime = "2026-06-01T09:00:00",
            messageToDoctor = "Hello", consultationReason = "Check-up"
        )
    )

    @Test fun `ModifyAppointmentRequest round-trips`() = assertRoundTrip(
        ModifyAppointmentRequest(
            dateTime = "2026-06-02T10:00:00",
            messageToDoctor = "Reschedule", consultationReason = "Follow-up"
        )
    )

    @Test fun `UpdateProfileRequest round-trips`() = assertRoundTrip(
        UpdateProfileRequest(
            firstName = "Sophie", lastName = "Müller", phone = "+352 621 000",
            gender = "Female", dateOfBirth = "1990-03-14",
            cnsNumber = "1990 0314 0001", language = "en"
        )
    )

    @Test fun `ChangeEmailRequest round-trips`() = assertRoundTrip(
        ChangeEmailRequest(newEmail = "new@test.lu", password = "secret")
    )

    @Test fun `ChangePasswordRequest round-trips`() = assertRoundTrip(
