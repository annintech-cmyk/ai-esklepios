...
class LogoutUseCaseTest {
    private class FakeAuthRepository : AuthRepository {
        ...
        override suspend fun login(
            email: String,
            password: String,
        ): Result<User> = Result.failure(Exception())
    ...
    @Test
    fun `logout success clears token and all user-scoped tables`() =
        runTest {
    ...
    @Test
    fun `logout failure propagates to caller`() =
        runTest {
            val repo =
                FakeAuthRepository().also {
    ...
    @Test
    fun `isLoggedIn is false after successful logout`() =
        runTest {
            val repo = FakeAuthRepository()
            val useCase = LogoutUseCase(repo)
