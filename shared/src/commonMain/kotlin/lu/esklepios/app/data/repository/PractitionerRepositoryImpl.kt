...
class PractitionerRepositoryImpl(
    ...
    private fun lu.esklepios.app.db.PractitionerEntity.toDomain(): Practitioner {
        ...
        val schedule = runCatching { json.decodeFromString<List<ScheduleEntry>>(scheduleJson) }.getOrDefault(emptyList())
        val paymentMethods = runCatching { json.decodeFromString<List<String>>(paymentMethodsJson) }.getOrDefault(emptyList())
        val diplomas = runCatching { json.decodeFromString<List<String>>(diplomasJson) }.getOrDefault(emptyList())
        return Practitioner(
            id = id,
            firstName = firstName,
            lastName = lastName,
