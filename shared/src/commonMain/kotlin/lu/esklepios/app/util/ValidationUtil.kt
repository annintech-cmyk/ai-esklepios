...
object ValidationUtil {
     ...
     */
    fun isValidEmail(email: String): Boolean = email.isNotBlank()
        && email.contains("@")
        && email.contains(".")
     ...
     */
    fun emailsMatch(first: String, confirm: String): Boolean =
        confirm.isNotBlank() && confirm == first

    /** `true` iff [confirmPassword] is non-blank and exactly equals [newPassword]. */
    fun passwordsMatch(newPassword: String, confirmPassword: String): Boolean =
     ...
     */
    fun passwordCriteria(password: String): Set<PasswordCriterion> = buildSet {
