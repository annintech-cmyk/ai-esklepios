...
File: `shared/src/commonMain/kotlin/lu/esklepios/app/data/network/models/<Name>Request.kt`

```kotlin
package lu.esklepios.app.data.network.models
...
File: `shared/src/commonMain/kotlin/lu/esklepios/app/data/network/models/<Name>Response.kt`
...
### 4. Implement in ApiServiceImpl
In `shared/src/commonMain/kotlin/lu/esklepios/app/data/network/ApiServiceImpl.kt`:
...
## Authentication
All endpoints are authenticated by default (the Ktor BearerTokenPlugin handles the header). For public endpoints (e.g., `/auth/login`), the Ktor `BearerTokenPlugin` configuration should skip token injection for those specific paths.
