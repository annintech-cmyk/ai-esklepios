...
class HomeViewModelTest {
    ...
    @Test
    fun `toggleFilter adds filter to selectedFilters`() = runTest {
        viewModel.toggleFilter("Cardiology")

        viewModel.uiState.test {
            val state = awaitItem()
            assertTrue(state.selectedFilters.contains("Cardiology"))
            cancelAndIgnoreRemainingEvents()
        }
    }

    @Test
    fun `toggleFilter removes filter when already selected`() = runTest {
        viewModel.toggleFilter("Cardiology")
        viewModel.toggleFilter("Cardiology")
