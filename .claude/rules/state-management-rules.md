...
### Rule SM-12: Main Thread UI Updates — Use @MainActor + FlowWatcher
`@Published` mutations from KMM StateFlow observations use `Task { @MainActor [weak self] in ... }` inside the FlowWatcher callback. The wrapper class itself is annotated `@MainActor`. `DispatchQueue.main.async { }` is **replaced** by this pattern:
...
See `.claude/rules/flow-watcher-rules.md` for the full spec.
