...
Each user-facing screen of eSklepios must be present and functional on both Android and iOS. ⚠ Screen filenames currently differ between platforms for two entries — until a rename is performed, document the pair explicitly in the table below.
...
| Practitioner List / Search Results | `PractitionerListScreen.kt` | `PractitionerListView.swift` / `SearchResultsView.swift` |
| Practitioner Detail | `PractitionerDetailScreen.kt` | `PractitionerDetailView.swift` |
| Book Appointment | `BookingScreen.kt` | `BookAppointmentView.swift` |
